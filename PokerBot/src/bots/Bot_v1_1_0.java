...

{
	...
	public void run()
	{
		try {
			...
			JFrame frame = new JFrame( " - Bot v1.0.0" );															// GUI
			...
			while ( true )
			{
				...
				if ( Other.compare(f1, PICTURE_BUTTON_FOLD, 0.65) || Other.compare(f2, PICTURE_BUTTON_CKECK_CALL, 0.65) || Other.compare(f3, PICTURE_BUTTON_BET_RAISE, 0.65) )
				{
					...
					try {
						hh = ParserCreatorWinnerPoker4Tables.parserMainCWP( hhFile, "Hold'Em", "Fixed Limit", 9, namePlayerYou, PICTURE_SEATS, SPACE_SEATS);
